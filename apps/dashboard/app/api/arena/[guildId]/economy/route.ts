...
export async function POST(req: Request, { params }: { params: { guildId: string } }) {
  ...
  const { action, itemId, rarities, count } = (await req.json()) as { action?: string; itemId?: string; rarities?: string[]; count?: number };
  ...
  try {
    switch (action) {
      ...
      case "wheelBulk":
        return NextResponse.json(await botApi.arenaWheelBulk(g, u, count ?? 1));
      case "upgrade":
